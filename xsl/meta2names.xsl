...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="getnames">
        ...
        <xsl:for-each select="collection($collquery)">
            <xsl:message>INFO: extracting names from <xsl:value-of select="document-uri(.)"/></xsl:message>
            <xsl:apply-templates select="./descendant-or-self::info[@type='isaw']/photographer"/>
            <xsl:apply-templates select="./descendant-or-self::agent"/>
        </xsl:for-each>
    </xsl:template>
